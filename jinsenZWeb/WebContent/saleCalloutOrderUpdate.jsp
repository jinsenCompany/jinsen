...
function not(){
	var saleCalloutOrder=document.getElementById("saleCalloutOrder").value;
	$.ajax({
        url:"salaryServlet?action=worksaleyesorno&type=not",
        data:{
...
<main>
    ...
    <tbody>
     ...
     <td>口径(厘米):<span></span></td><td colspan="2"><input style="border:0px;background-color: transparent;font-size:20px;text-align:center" type='text' name='tradius' id='tr<%=i%>' value="${b.getTradius()}"></td></tr>
        ...
        <p class="table_p"><span>请选择检尺员信息</span></p>
        <table style="margin-left:auto; margin-right:auto;background-color:#CCFFFF;">
        <% int j=1;%>
        <tr>
        <c:forEach items="${surveyor}" var="c">    
        <td id="<%=j%>" display:block; style='font-size:20px;'><input type="checkbox" style='width:25px;height:25px;' value="<%=j%>">
        <input style="border:0px;background-color: transparent;font-size:20px;" name="surveyor" id="surveyor<%=j%>" value="${c.getSurveyor()}">
          <%j++; %>
           </c:forEach>
           </tr>
            </tbody>
        </table>
        <p class="table_p"><span>请通知货场管理员:</span></p>
        <table style="margin-left:auto; margin-right:auto;background-color:#CCFFFF;">
    ...
    <div class=" but_p" style="float:center;">
     <button style="width:150px;height:50px" class="but_save" type="button"  onclick="yes()" value="同意出仓">同意出仓</button>
     <button style="width:150px;height:50px" class="but_save" type="button"  onclick="not()" value="不同意出仓">不同意出仓</button>
