...
function aaad()
{
 	var str="<tr id="+k+" display:block;><td style='font-size:20px;'><input type='checkbox' style='width:20px;height:20px;' value="+k+">树材种<span></span>"
          ...
          +"长度(米)<span></span><input style='width: 180px' name='tlong' id='tl"+k+"' value='  至    米'>"
    ...
    $.ajax({
        ...
        success: function (data) {
        	...
        	if(data>0)
        		{
        	        alert("添加成功！");
        	        window.location.href = 'saleCalloutOrderShenheModer.jsp';
        		}
        	else{
        		 alert("添加失败");
        		 window.location.href = 'saleCalloutOrderShenheModer.jsp';
...
<main>
    ...
    <tbody>
    <tr>
       <td>时间:</td><td><input type="date" name="callidtime" id="callidtime" style="border:0px;background-color: transparent;font-size:20pt;" value="<%=s.getCallidtime() %>"></td>
       </tr>
       <tr>
       <td>合同编号</td><td><input type="text" name="contractnum" id="contractnum" value="<%=s.getContractnum() %>"></td>
       <td>购货人:</td><td><input type="text" name="demander" id="demander" value="<%=s.getDemander() %>"></td>
       <!--  <td>供货地点</td><td><input type="text" name="yard" id="yard"></td>-->
       </tr>
       <tr>
       <td>供货地点:</td>
       <td><input type="text" name="yard" id="yard" value="<%=s.getYard() %>"></td>
       <td>供货分区:</td><td><input type="text" name="section" id="section" value="<%=s.getSection() %>"></td>
       </tr>
       <tr>
    ...
    <table id="table5" class="table2">
     ...
     <tr id="<%=i%>" display:block;><td style='font-size:20px;'>
           ...
           <option value='衫短材' <c:if test="${b.getTreetype()eq'衫短材'}">selected='selected'</c:if> >--衫短材--</option>
           <option value='杉木兜' <c:if test="${b.getTreetype()eq'杉木兜'}">selected='selected'</c:if> >--杉木兜--</option>
           <option value='杉直柄' <c:if test="${b.getTreetype()eq'杉直柄'}">selected='selected'</c:if> >--杉直柄--</option>
           <option value='特种材' <c:if test="${b.getTreetype()eq'特种材'}">selected='selected'</c:if> >--特种材--</option>
           <option value='其他' <c:if test="${b.getTreetype()eq'其他'}">selected='selected'</c:if> >--其他--</option>
                                  ...
                                  口径(厘米)<span></span><input type='text' style='width: 180px' name='tradius' id='tr<%=i%>' value="${b.getTradius()}"></td></tr>
          <%i++; %>
           </c:forEach>
            </tbody>
            </table> 
