...
public class cutnumServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //通过采伐证号搜索采伐证信息
        	 ...
        	 sql="select company,certificatenum,cutnum,cutaddress,cutvillage,quartel,largeblock,smallblock,volume from cutnum";
      ...
      //管理部主页查看输入采伐证号详细信息
        ...
        //将采伐证信息显示再创建工程包页面(未选择工程包的采伐证号)
        	...
        	sql="select cutnum,certificatenum,starttime,endtime,company,cutarea,volume from cutnum WHERE proj_package_id=0";
        ...
        /*else if("addproject".equals(action)) {
    		...
    		try {
    			...
    			for (FileItem item : list) {
    				...
    				if (item.isFormField()) {
    					String name = item.getFieldName();//普通输入项username
    					if("projectPackageid".equals(name)) {
    						pk.setprojectPackageid(Double.parseDouble(item.getString("UTF-8")));
    						projectPackageid=Double.parseDouble(item.getString("UTF-8"));
    					}
    						...
    						java.sql.Date begin = new java.sql.Date(d.getTime()); 
    						pk.setProjpackageStarttime(begin);
    					}
        ...
        //在更新工程包页面显示被当前工程包选取的采伐证号
        	...
        	double projectPackageid = Double.parseDouble(request.getParameter("projectPackageid"));
            sql="select cutnum,certificatenum,starttime,endtime,company,cutarea,volume from cutnum WHERE proj_package_id="+projectPackageid+"";
              List<cutnum> cw=cnd.findCutnumproject(sql);
        	//System.out.println("...."+cw.size() + "...");
        	mapper.writeValue(response.getWriter(), cw);
        }
      ...
      //管理经理审核以补充材料的申请书
        ...
        //拨交查看采伐证材料
        	...
        	double projectPackageid = Double.parseDouble(request.getParameter("projectPackageid"));
        	sql="select c.cutnum,c.certificatenum,c.starttime,c.endtime,c.company,c.cutarea,c.volume from cutnum as c join cutnum_status as s on c.cutnumid=s.cutnumid WHERE c.proj_package_id="+projectPackageid+" and s.`status`>=0";
