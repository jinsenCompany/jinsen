...
public class workpageDaoImpl implements workpageDao {
	...
	@Override
	public int addyardSection(yardManage cp) {
		String sql = "insert into yard_section values(?,?)";
		int res=0;
		try {
			res=dbc.doUpdate(sql, new Object[] {cp.getYardname(),cp.getSection()});
		}
		catch (Exception e) 
		{
			e.printStackTrace();
	...
	@Override
	public int addSurveyor(surveyor cp) {
		String sql = "insert into surveyor_info values(?,?)";
		int res=0;
		try {
			res=dbc.doUpdate(sql, new Object[] {cp.getSurveyorid(),cp.getSurveyor()});
		}
		catch (Exception e) 
		{
			e.printStackTrace();
	...
	@Override
	public List<surveyor> findSurveyor(String sql) {
		List<surveyor> addrList=new ArrayList<surveyor>();
		try {
			ResultSet rs=dbc.doQuery(sql, new Object[] {});
			while(rs.next()) {
				surveyor a=new surveyor();
				a.setSurveyorid(rs.getInt(1));
				a.setSurveyor(rs.getString(2));
				addrList.add(a);
			}
		}catch(Exception e) {
			e.printStackTrace();
	...
	@Override
	public yardManage findyardmanager(String sql) {
		yardManage a=new yardManage();
		try {
			ResultSet rs=dbc.doQuery(sql, new Object[] {});
			while(rs.next()) {
				
			    a.setYardmanager(rs.getString(1));;;
			}
		}catch(Exception e) {
			e.printStackTrace();
		}finally {
