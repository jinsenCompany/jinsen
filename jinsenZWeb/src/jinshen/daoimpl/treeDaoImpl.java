...
public class treeDaoImpl implements treeDao {
	...
	@Override
	public int updateTree1(tree c,double workid) {
		int res=0;
		try {
			String sql="update tree set treetype=?,tlong=?,tradius=?,num=?,tvolume=? where workid="+workid+"";
			res=dbc.doUpdate(sql, new Object[] {c.getTreetype(),c.getTlong(),c.getTradius(),c.getNum(),c.getTvolume()});
		}catch(Exception e) {
			e.printStackTrace();
		}finally {
			dbc.close();
	...
	public int addOutyard(outyard cp) {
		...
		try {
			res=dbc.doUpdate(sql, new Object[] {cp.getWorkid(),cp.getYarddate(),
					cp.getCarNumber(),cp.getYard(), cp.getCutnum(), cp.getSurveyor(),cp.getToltree(),cp.getTolstere(),cp.getContractnum(),cp.getSaleCalloutorderid(),cp.getSection()});
		}
		catch (Exception e) 
		{
			e.printStackTrace();
	...
	public outyard findOutSingle(String sql) {
		...
		try {
			...
			if(rs.next()) {
				addr.setWorkid(rs.getDouble(1));
				addr.setYarddate(rs.getTimestamp(2));
				addr.setCarNumber(rs.getString(3));
				addr.setYard(rs.getString(4));
				addr.setCutnum(rs.getString(5));
	...
	public outyard searchoutyard() {
		String sql="select contractnum,workid,sale_callout_orderid,yard,section,cutnum,yarddate,carNumber,surveyor from outyard";
    	ResultSet rs = null;
    	outyard cn=new outyard();
    	try {
