...
function yes(){
	...
	$.ajax({
		url:"cutnumServlet?action=applyfeedback&type=yes",
        //url:"cutnumServlet",
        data:{
        ...
        success: function (data) {
        	if(data>0){
        		alert("反馈成功");
        		window.location.href = 'cutnumApplyseeAassis.jsp';
        	}
        	else{
        		alert("反馈失败");
        		window.location.href = 'cutnumApplyseeAassis.jsp';
...
function not(){
	...
	$.ajax({
        url:"cutnumServlet?action=applyfeedback&type=not",
        data:{
...
<div id="sidebar"><a href="#" class="visible-phone"><i class="icon icon-home"></i> 仪表盘</a>
  <ul>
    <li class=active><a href="cutnumApplyseeAassis.jsp"><i class="icon icon-home"></i> <span>审核采伐证申请</span></a> </li>
...
<main>
    <div class="find-top">
        <p class="p-tail"><i
         class="i-tail"></i> 该页面主要是管理部门审核采伐证材料页面</p>
    </div>
    <div class="find-top1">
    ...
    <td>请添加补充材料:</td><td>${replenish}<a href="DownfileServlet?action=applyfile&filename=<%=f.getReplenishpath() %>">下载</a></td>
     ...
     <div> 
    <div><p align="center" class="table_p" style="margin-top: 65px;"><span style="font-size:30px">是否办理</span></p></div>
    <div class=" but_p" style="float:center;">
    <button class="but_save" type="button" onclick="yes()" value="审核通过">审核通过</button>
    <button class="but_save" type="button"  onclick="not()" value="审核不通过">审核不通过 </button>
